...
```
python run_report.py --write-pptx "CA_bitumen_market_25_05_2026.pptx"
```
...
Automated data collection (runs before report)
    │
    ├── currency_scraper.py   ← CBR API: daily RUB/KZT/UZS/KGS/TJS → currency_rates.xlsx
    ├── weather_scraper.py    ← Playwright: rp5.ru, 13 cities × 4 countries → weather_central_asia.xlsx
    └── exchange_scraper.py   ← UzEx (HTML), ETS + CCX (Playwright) → uz_exchange.xlsx, kz_exchange.xlsx
...
**Quarterly report** (`generate_quarterly_slides.py`): same approach, runs per-country on aggregated PQ data from a quarterly workbook.

---

## Code Samples

The full pipeline runs on real client data and isn't published wholesale. A few representative pieces are included to illustrate the different types of processing involved:

- `data-collection/currency_scraper.py` — API scraping + Excel write via xlwings
- `weekly/write_analytics.py` — parsing generated analytics text and inserting it into PPTX TextBoxes (XML manipulation via lxml)
- `pq_examples/price_consolidation.txt` — Power Query (M) example: merging price series across countries and currencies into one long table
- `pq_examples/supply_demand_balance.txt` — Power Query (M) example: production + imports − exports = consumption, anchored so no (country, product, year) row is silently dropped

These two `pq_examples/` files are adapted from the real production queries (renamed identifiers, no live file paths) for public presentation — not literal copies of the working files, which reference live Excel data and stay private.
...
|--------|--------|------|--------|
| `currency_scraper.py` | CBR API (xml) | Daily rates: RUB, KZT, UZS, KGS, TJS vs USD | `currency_rates.xlsx` |
| `weather_scraper.py` | rp5.ru (Playwright) | Weekly temp stats: 13 cities × 4 countries | `weather_central_asia.xlsx` |
| `exchange_scraper.py` | uzex.uz (HTML) | Uzbekistan commodity exchange: bitumen + mazut deals | `uz_exchange_bitumen_mazut.xlsx` |
| `exchange_scraper.py` | ets.kz (Playwright) | Kazakhstan energy exchange: bitumen trade volumes | `kz_exchange.xlsx` |
| `exchange_scraper.py` | ccx.kz (API + Playwright) | Kazakhstan Central Asian exchange: bitumen prices | `kz_exchange.xlsx` |

### Excel source files (read by collect_data.py)

| File | What | Format |
|------|------|--------|
| `prices_consolidated.xlsx` | Producer prices KAZ + UZB exchange + RF refineries | Excel PQ |
| `rail_deliveries.xlsx` | Railway deliveries from RF by country and refinery | Excel PQ |
| `currency_rates.xlsx` | Weekly exchange rates | Excel PQ |
| `kz_producer_prices.xlsx` | Kazakhstan producer monitoring prices | Excel |
