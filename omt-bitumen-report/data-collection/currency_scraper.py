...
TARGET_PATH = Path(os.getenv("CURRENCY_XLSX_PATH", Path(__file__).parent / "currency_rates.xlsx"))
...
def fetch_cbr_rates(date: datetime.date) -> dict:
    ...
    if missing:
        print(f"Currencies missing from CBR response: {missing}")
        sys.exit(1)
    ...
    return {
        "RUB":     usd,           # USD/RUB: rubles per 1 dollar
        "KZT":     usd / kzt,     # USD/KZT: tenge per 1 dollar
        "UZS":     usd / uzs,     # USD/UZS: som per 1 dollar
        "KGS":     usd / kgs,     # USD/KGS: som per 1 dollar
        "TJS":     usd / tjs,     # USD/TJS: somoni per 1 dollar
        "KZT_RUB": 1.0 / kzt,     # KZT/RUB: tenge per 1 ruble
    }
...
def main():
    ...
    try:
        ...
        for sheet_name, (rate_key, decimals) in SHEET_MAP.items():
            ...
            print(f"  [{sheet_name}] added {len(missing_dates)} days")

        wb.save()
        print(f"\nFile saved: {TARGET_PATH.name}")
        ...
        if wb is not None:
            try:
                ...
                pass
        try:
            app.kill()  # force-close the Excel process
        except Exception:
            pass
